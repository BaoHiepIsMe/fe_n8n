...
import React, { useState, useRef, useCallback } from 'react';
import '../styles/dashboard.css';
import { documentAPI, authAPI } from '../lib/api'; // Added authAPI import
import { useAuth } from '../contexts/AuthContext';
import { useDocuments } from '../contexts/DocumentContext';
...
const Dashboard = () => {
    ...
    const handleFile = useCallback(async (file) => {
        ...
        setUploadResult({
            success: true,
            processing: true,
            message: 'Đang kiểm tra tài khoản...', // Changed message
            fileName: file.name,
        });

        try {
            // [NEW CODE] 1. Kiểm tra trạng thái block của user trước khi upload
            // Gọi API lấy thông tin profile mới nhất để check is_blocked
            const profileResponse = await authAPI.getProfile(user.id);
            
            // Giả sử API trả về data có trường is_blocked. Cần đảm bảo authAPI.getProfile trả về đúng cấu trúc.
            // Nếu profileResponse.data hoặc profileResponse trực tiếp là object profile
            const userProfile = profileResponse.data || profileResponse; 

            if (userProfile && userProfile.is_blocked === 1) {
                 setUploadResult({
                    success: false,
                    processing: false,
                    message: 'Tài khoản của bạn đã bị CHẶN. Vui lòng liên hệ quản trị viên để biết thêm chi tiết.',
                });
                if (fileInputRef.current) {
                    fileInputRef.current.value = '';
                }
                return; // Dừng upload ngay lập tức
            }
             
             // Cập nhật trạng thái loading sau khi check xong
            setUploadResult(prev => ({ ...prev, message: 'Đang xử lý file...' }));


            // [OLD CODE] 2. Gửi file lên n8n và đợi response (như code HTML của bạn)
            const result = await documentAPI.uploadToN8n(file, user.id);
                            ...
                            {uploadResult.processing ? (
                                <>
                                    ...
                                    <h3 style={{
                                        ...
                                        {uploadResult.message || 'Đang xử lý'} {/* Hiển thị message cập nhật */}
                                    </h3>
                                    <p style={{
                                        fontSize: '16px',
                                        color: 'var(--text)',
